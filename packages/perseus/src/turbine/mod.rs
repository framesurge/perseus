//! The internals of Perseus' state generation platform. This is not responsible for the reactivity of state,
//! or any other browser-side work. This is responsible for the actual *generation* of state on the engine-side,
//! at both build-time and request-time.
//!
...
impl<M: MutableStore, T: TranslationsManager> Turbine<M, T> {
    /// Updates some internal fields of the turbine by assuming the app has been built in the past.
    /// This expects a number of things to exist in the filesystem. Note that calling `.build()` will
    /// automatically perform this population.
    pub async fn populate_after_build(&mut self) -> Result<(), ServerError> {
        ...
        for locale in self.locales.get_all() {
            // IMPORTANT: A global state that doesn't generate at build-time won't have a corresponding file!
            let res = self.immutable_store.read(
