...
impl<M: MutableStore, T: TranslationsManager> Turbine<M, T> {
    ...
    ///
    /// If a translator and translations string is provided, it will be assumed to be of the
    /// correct locale, and will be injected into the page. A best effort should
    /// be made to provide translations here.
    ///
    /// # Pitfalls
    /// If a translations string is provided here that does not match with the locale
    /// actually being returned (i.e. that which the client will infer), there will be
    /// a mismatch between the translations string and the locale, which can only be
    /// rectified by the user manually switching to another locale and back again.
    /// Please ensure the correct translations string is provided here!
