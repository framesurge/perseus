...
pub struct Template<G: Html> {
    ...
    /// Whether or not this template is a capsule. It's necessary to delineate
    /// this so that capsules can be rendered before pages, in order that state
    /// flows work properly.
    is_capsule: bool,
...
// We use a macro to implement the same methods on both `Template` and
// `Capsule`, enabling the better ergonomics of the two being synonyms with one
// property predefined in a different way on each (viz. `is_capsule`)
    ...
    ($name:ty) => {
        impl<G: Html> $name {
            ...
            pub fn render_for_template_client<'a>(
                ...
                props: PageProps,
                cx: Scope<'a>,
                // Taking a reference here involves a serious risk of runtime panics, unfortunately
                // (it's simpler to own it at this point, and we clone it anyway
                // internally)
            ...
            pub async fn get_build_state(
                ...
                if let Some(get_build_state) = &self.get_build_state {
                    ...
                    match res {
                        Ok(res) => Ok(res),
                        Err(GenericErrorWithCause { error, cause }) => {
                            Err(ServerError::RenderFnFailed {
            ...
            ///
            /// This will automatically add `__capsule/` to the front of capsules.
            pub fn get_path(&self) -> String {
                let path = if self.path == "index" {
                    String::new()
                } else {
                    self.path.clone()
                };
                if self.is_capsule {
                    format!("__capsule/{}", path)
                } else {
                    path
                }
            }
            ...
            pub fn revalidate_after<I: PerseusDuration>(mut self, val: I) -> Self {
                let computed_duration = match val.into_computed() {
                    Ok(val) => val,
                    // This is fine, because this will be checked when we try to build the app (i.e.
                    // it'll show up before runtime)
...
///
///
/// *Note: this is almost exactly the same as [`Template`].*
...
impl<G: Html> Capsule<G> {
    /// Creates a new [`Capsule`]. By default, this has absolutely no
    /// associated data. If rendered, it would result in a blank screen.
    pub fn new(path: impl Into<String> + std::fmt::Display) -> Self {
        Self {
            path: path.to_string(),
            is_capsule: true, // Different to `Template`!
            template: Box::new(|cx, _| sycamore::view! { cx, }),
            // Unlike `template`, this may not be set at all (especially in very simple apps)
            #[cfg(not(target_arch = "wasm32"))]
            head: Box::new(|cx, _| sycamore::view! { cx, }),
...
impl_template!(Template<G>);
impl_template!(Capsule<G>);
