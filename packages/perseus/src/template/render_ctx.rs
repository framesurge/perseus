...
use super::{ArcTemplateMap, ResolvedWidgetDependency, TemplateState, TemplateStateWithType};
use crate::errors::*;
...
/// An accumulator of the information about widgets. This will be provided to the context when
/// rendering a page/widget while in accumulator mode, allowing each widget to resolve itself
/// to the information needed to actually render it.
///
/// This is used at request-time so we can render the dependencies of a page layer-by-layer
/// (since rendering them all at once is impossible, as we don't know what the dependencies
/// of a widget are until we know its state, since its dependencies could well be state-dependent).
#[cfg(not(target_arch = "wasm32"))]
#[derive(Debug, Clone)]
pub(crate) struct DependencyAccumulator {
    /// A map of widget path to resolution. The paths in here will be template-complete,
    /// i.e. they will be prefixed with `__capsule/`.
    resolutions: Rc<RefCell<HashMap<String, ResolvedWidgetDependency>>>,
    /// The render configuration of the app. Of course, this is only available after build-time.
...
impl DependencyAccumulator {
    /// Creates a new dependency accumulator.
    pub(crate) fn new(
        render_cfg: Arc<HashMap<String, String>>,
        templates: ArcTemplateMap<SsrNode>,
        locales: Locales,
    ...
    /// Adds the given widget path to the current dependency accumulator by resolving
    /// which capsule it was generated by.
    pub(crate) fn add(
        &self,
        path: &str,
    ) -> Result<(), ServerError> {
        ...
        // If the path has already been registered, don't do so again (note that this would only
        // happen if the same widget is used multiple times on the same page, which is perfectly valid)
        ...
        // Now get a route verdict to determine the capsule this widget path maps to
        let localized_path = format!("{}/{}", self.curr_locale, path);
        ...
        let resolved_dep = match verdict {
            RouteVerdictAtomic::Found(route_info) => {
                ResolvedWidgetDependency {
            ...
            RouteVerdictAtomic::LocaleDetection(_) => return Err(ServerError::ResolveDepLocaleRedirection {
                locale: self.curr_locale.to_string(),
        ...
        resolutions.insert(path.to_string(), resolved_dep);
...
pub(crate) enum RenderMode {
    ...
    Request {
        ...
        unresolved_widget_accumulator: Rc<RefCell<Vec<String>>>
    },
...
impl RenderCtx {
    ...
    pub(crate) fn server(global_state: TemplateState, mode: RenderMode,) -> Self {
        Self {
