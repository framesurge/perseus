...
fn engine_widget<G: Html>(cx: Scope, path: PathWithoutLocale) -> View<G> {
    ...
    match &reactor.render_mode {
        RenderMode::Build {
            ...
            // Check if we're in the render config (which will just contain widgets at this point,
            // since they're built first, and the rendering we're in now for templates is executed afterward)
            if let Some(capsule_name) = widget_render_cfg.get(&*path) {
                ...
                if capsule.uses_request_state() || capsule.revalidates() {
                    *render_status.borrow_mut() = RenderStatus::Cancelled;
                    ...
                    // This won't panic, because the reactor has been fully instantiated with a translator on
                    // the engine-side (unless we're in an error page, which is totally invalid)
                    let locale = reactor.get_translator().get_locale();
                    ...
                    match capsule.render_widget_for_template_server(PathMaybeWithLocale::new(&path, &locale), state, cx) {
                        Ok(view) => view,
                        Err(err) => {
                            *render_status.borrow_mut() = RenderStatus::Err(ServerError::ClientError(err));
                            View::empty()
                        }
                    }
                }
            } else {
                // This widget will be incrementally generated (TODO should we try to build it now?). It could
                // also just not exist, but we can't confirm that until request time (since incremenally generated
                // page could also be invalid). Remember that this will only get through if the user has explicitly
                // allowed deferring renders.
                *render_status.borrow_mut() = RenderStatus::Cancelled;
                View::empty()
            }
        },
            ...
            match widget_states.get(&full_path) {
                Some(res) => match res {
                    ...
                    Ok((capsule_name, state)) => {
                        ...
                        let capsule = match templates.get(capsule_name) {
                            Some(capsule) => capsule,
                            // We successfully got a proper routing match for this, so finding that the
                            // capsule then doesn't exist should not be possible
                            None => unreachable!(),
                        };
                        // Use that to render the widget for the server-side (this should *not* create a new reactor)
                        match capsule.render_widget_for_template_server(full_path, state.clone(), cx) {
                            Ok(view) => view,
                            // We'll render any errors to the whole widget, even if they might be internal (but
                            // they *really* shouldn't be, since those should've been handled when trying to fetch
                            // the state, as there's no active syste etc. on the engine-side)
                            Err(err) => error_views.handle_widget(&err, cx)
                        }
                    },
                    // We're to render an error page with the given error data (which will not impact the rest of the page).
                    // Since this whole `Request` variant can only happen for initial loads, and since this is a `ServerError`,
