use darling::{FromDeriveInput, FromField, ToTokens, ast::Data};
...
pub struct ReactiveStateDeriveInput {
    /// If specified, a type alias will be created for the final reactive `struct` for ease of
    /// reference.
    #[darling(default)]
    alias: Option<Ident>,
...
pub struct ReactiveStateField {
    /// Whether or not we should expect the annotated field to be able to made reactive itself,
    /// enabling nested reactivity.
    #[darling(default)]
    nested: bool,
...
/// The underlying implementation of the `ReactiveState` derive macro, which implements the traits
/// involved in Perseus' reactive state platform, creating an intermediary reactive struct using `RcSignal`s
/// and a final one using `&'cx Signal`s, where `cx` is a Sycamore scope lifetime.
pub fn make_rx_impl(input: ReactiveStateDeriveInput) -> TokenStream {
    ...
    // Now go through them and create what we want for both the intermediate and the reactive `struct`s
    ...
    for field in fields.iter() {
        ...
        if field.nested {
            ...
            unrx_field_makers.extend(quote! { #field_ident: self.#field_ident.clone().make_unrx(), });
        } else {
            intermediate_fields.extend(quote! {
                #field_attrs
            ...
            intermediate_field_makers.extend(quote! { #field_ident: ::sycamore::prelude::create_rc_signal(self.#field_ident), });
            ref_field_makers.extend(quote! { #field_ident: ::sycamore::prelude::create_ref(cx, self.#field_ident), });
            unrx_field_makers.extend(quote! { #field_ident: (*self.#field_ident.get_untracked()).clone(), }); // All fields must be `Clone`
        }
    }

    let ReactiveStateDeriveInput { ident, vis, attrs: attrs_vec, alias, .. } = input;
    let mut attrs = quote!();
    for attr in attrs_vec.iter() {
        attrs.extend(attr.to_token_stream());
    }
    let intermediate_ident = Ident::new(&(ident.to_string() + "PerseusRxIntermediate"), Span::call_site());
    let ref_ident = Ident::new(&(ident.to_string() + "PerseusRxRef"), Span::call_site());

    // Create a type alias for the final reactive version for convenience, if the user asked for one
