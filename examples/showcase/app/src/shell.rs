...
// This fetches the static resources for a page and renders it
#[macro_export]
macro_rules! app_shell {
    ({
        name => $name:expr,
        $(props => $props:ty,)?
    ...
    }) => {
        {
            ...
            ::wasm_bindgen_futures::spawn_local(cloned!((container) => async move {
                ...
                let page_data_str = $crate::shell::fetch(format!("/.perseus/page/{}", $name)).await;
                ...
                let page_data = serde_json::from_str::<$crate::serve::PageData>(&page_data_str);
                ...
                let state = match page_data.state {
                    Some(state_str) => {
                        ...
                        $(
                            // Serialize that state into a valid properties struct
