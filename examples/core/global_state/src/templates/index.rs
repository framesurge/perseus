...
// Note that this template takes no state of its own in this example, but it certainly could
#[perseus::template_rx]
pub fn index_page<'a, G: Html>(cx: Scope<'a>) -> View<G> {
    // We access the global state through the render context, extracted from Sycamore's context system
